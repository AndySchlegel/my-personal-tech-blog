# main.tf - S3 bucket for blog assets (Lightsail-only version)
#
# Simplified version of the EKS S3 module. This bucket stores all images
# uploaded through the admin dashboard and Polly audio files.
#
# Security model:
#   ALL public access is blocked. No one can access files directly via S3 URLs.
#   Files are served exclusively through CloudFront via Origin Access Control (OAC).
#   This means: users see https://techblog.aws.his4irness23.de/images/photo.jpg
...
#
# Without CORS: the admin dashboard (running on techblog.aws.his4irness23.de)
# would be blocked by the browser when trying to upload images to S3,
# because S3 is a different origin (different domain).
#
# Lightsail-only version: only the Lightsail domain + localhost for dev.
# The EKS domain_name variable was removed since this root is Lightsail-only.
